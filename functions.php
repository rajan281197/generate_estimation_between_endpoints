...
function user_follow_unfollow_api_callback() {
	...
	// register_rest_route('auth', '/unfollow', array(
	// 	'methods' => WP_REST_Server::CREATABLE,
	// 	'callback' => 'unfollow_user_By_user_id',
	// ));
...
/**
 * Registers a new user account in the context of a business using the WordPress REST API.
 *
 * @param WP_REST_Request $request_data The request object containing the user's registration data.
 *
 * @return WP_Error|WP_User WP_Error object if the registration fails, WP_User object if registration succeeds.
 */
// function follow_user_by_user_id(WP_REST_Request $request_data)
// {
// 	$response               = array();
// 	$user_id      			= sanitize_text_field($request_data['user_id']);
// 	$follow_user_id      	= sanitize_text_field($request_data['follow_user_id']);
//     $action 				= sanitize_text_field($request_data['action']);


// 	$get_user_by_id = get_user_by('id', $user_id);
// 	$get_follow_user_id = get_user_by('id', $follow_user_id);
	
// 	if(!$get_user_by_id)
// 	{
// 		wp_send_json_error( "User Not exist of $user_id ID", 400 ); // send an error response with HTTP 400 status code
// 	}

	
// 	if(!$get_follow_user_id)
// 	{
// 		wp_send_json_error( "Follow User Not exist of $follow_user_id ID", 400 ); // send an error response with HTTP 400 status code
// 	}

// 	if($user_id == $follow_user_id)
// 	{
// 		wp_send_json_error( "User ID & Follow User ID always a different.", 400 ); // send an error response with HTTP 400 status code
// 	} else {
// 		$test =  get_user_meta( $user_id, 'follower_list', true );
// 		print_r($test);
// 		$current_list = $test ? explode(',', $test) : array();

// 		if (!in_array($follow_user_id, $current_list)) {
// 			$current_list[] = $follow_user_id;
// 			$updated_value = implode(',', $current_list);
// 			update_user_meta($user_id, 'follower_list', $updated_value, $test);
			
// 		}
// 		// $string = implode(',', $test);
// 		// var_dump($test);
// 		// if ( ! get_user_meta( $user_id, 'follower_list' ) != $string ) {
// 		// 	update_user_meta( $user_id, 'follower_list', $follow_user_id,false );
// 		// }
// 	}
// 	return $response;
// }
